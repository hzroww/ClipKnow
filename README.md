...
> **还没录。** 这一节需要一段 2 分钟的录屏,我没法替你录(那是你的屏幕)。
>
> 录的时候建议按这条线走,正好覆盖四个卖点:
>
> 1. 打开新会话 → 开场页(**0:00–0:10**)
> 2. 贴一条 TikTok 链接问「这视频在讲什么」→ 看步骤条逐条出现、答案边生成边
>    渲染成 Markdown(**0:10–0:40**)
> 3. 追问「画面里有几个人」→ 命中 48 小时内的上传引用,7 秒出结果(**0:40–1:00**)
> 4. 换一个开放式问题「找几个做天文科普的博主」→ 看它连续搜、下钻、撞闸门后
>    如实交代哪部分没查成(**1:00–1:50**)
> 5. 切回上一个会话 → 历史还在(**1:50–2:00**)
>
> `brew install --cask kap`,导出 GIF,放到 `docs/demo.gif`,然后把这段话换成
> `![演示](docs/demo.gif)`。GIF 超过 10MB 就传 YouTube 放链接。

在录屏做出来之前,**第四节是一条真实请求的完整逐步轨迹**——从事件流原样导出的,
比 GIF 更能说明它到底怎么工作。
