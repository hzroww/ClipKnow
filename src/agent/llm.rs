...
impl AnthropicClient {
    ...
    pub fn from_env() -> Result<Self> {
        let key = crate::env_var("ANTHROPIC_API_KEY")
            .ok_or(ClipKnowError::MissingEnv("ANTHROPIC_API_KEY"))?;
        // 每家一个独立的变量。共用一个 CLIPKNOW_MODEL 会出事：
        // 设了 claude-opus-5 之后再 --provider deepseek，模型名会被发给
        // DeepSeek，直接 400。
        let model = crate::env_var("ANTHROPIC_MODEL").unwrap_or_else(|| DEFAULT_MODEL.to_string());
...
impl DeepSeekClient {
    ...
    pub fn from_env() -> Result<Self> {
        let key = crate::env_var("DEEPSEEK_API_KEY")
...
pub fn build_client(explicit: Option<Provider>) -> Result<Box<dyn LlmClient>> {
    let has_deepseek = crate::env_var("DEEPSEEK_API_KEY").is_some();
    let has_anthropic = crate::env_var("ANTHROPIC_API_KEY").is_some();
